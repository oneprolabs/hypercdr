...
Kubernetes options:
  ...
  --base-url URL        Primary control plane URL used by users and agents
  --public-base-url URL       Optional public control plane URL used as Agent fallback
...
Docker options:
  ...
  --agent-private-endpoint URL Private WebSocket endpoint preferred by agents
  --agent-public-endpoint URL  Optional public WebSocket fallback endpoint
  --install-dir PATH              Persistent installation directory, default: /var/lib/hypercdr
  --registry REGISTRY          Optional OCI Registry prefix override.
  --registry-profile NAME      Registry profile; defaults to HCDR_ACTIVE_REGISTRY.
  --registry-config PATH       Registry profiles file.
  --registry-trust MODE        system (default) or private-ca
...
Recommended no-DNS Kubernetes flow:
  ...
  2. Run this script with --base-url https://<node-ip>:<node-port>.
...
if [[ -n "${public_base_url}" ]]; then
  [[ "${public_base_url}" =~ ^https?://[^/]+/?$ ]] || { echo "--public-base-url must be an http(s) origin without a path" >&2; exit 2; }
  public_base_url="${public_base_url%/}"
  agent_public_endpoint="${public_base_url/https:/wss:}"
...
preflight_docker_host() {
  ...
  [[ -d "${disk_path}" && -w "${disk_path}" ]] || {
    echo "The existing parent of the HyperCDR installation directory is not writable: ${disk_path}" >&2
    return 1
  }
  available_kb="$(df -Pk "${disk_path}" | awk 'NR==2 {print $4}')"
  [[ "${available_kb}" =~ ^[0-9]+$ ]] || {
    echo "Unable to determine free disk space for ${install_dir}." >&2
    return 1
  }
  if (( available_kb < 10 * 1024 * 1024 )); then
    echo "At least 10 GiB of free space is required on ${install_dir}; 100 GiB is recommended." >&2
    return 1
  fi
}
...
EOF
  ...
  if [[ -z "$node_port" ]]; then
    node_port="$(extract_port_from_url "$base_url" || true)"
  fi
  if [[ -z "$node_port" ]]; then
    node_port="30080"
  fi
...
helm ${helm_args[*]} upgrade --install ${RELEASE_NAME} ${CHART_DIR} \\
  ...
  --set-string global.publicBaseURL=${base_url} \\
...
run_docker() {
  ...
  if [[ -f "${install_dir}/release-token" ]]; then
    release_token="$(tr -d '\r\n' < "${install_dir}/release-token")"
  else
    release_token="$(openssl rand -hex 32)"
  fi
  if [[ -z "${registration_executor_token}" ]]; then
    if [[ -f "${install_dir}/registration-executor-token" ]]; then
      registration_executor_token="$(tr -d '\r\n' < "${install_dir}/registration-executor-token")"
    else
      registration_executor_token="$(openssl rand -hex 32)"
    fi
  fi
...
EOF
    ...
    printf '%s\n' "${release_token}" > "${install_dir}/release-token"
    chmod 600 "${install_dir}/release-token"
...
EOF
    ...
    if [[ "${ready}" != "true" ]]; then
      ...
      echo "          Check: cd ${install_dir} && docker compose ps" >&2
      echo "          Logs:  cd ${install_dir} && docker compose logs --tail=200" >&2
      exit 1
    fi
