...
export default function RealRestorePointPage({
  ...
  const restorePointTableColumns = useMemo<HyperTableColumn<Row>[]>(() => [
    ...
    ...(visibleColumns.includes('time') ? [{ id: 'time', header: 'Restore Point', accessorFn: (row: Row) => row.time, size: 190, minSize: 160, maxSize: 260, cell: (info: any) => <span>{info.row.original.time}</span>, meta: { kind: 'primary', title: (row: Row) => row.time } }] : []),
    ...(visibleColumns.includes('storage') ? [{ id: 'storage', header: 'Repository', accessorFn: (row: Row) => row.storage, size: 150, minSize: 120, maxSize: 240, cell: (info: any) => <span>{info.row.original.storage}</span>, meta: { kind: 'secondary', title: (row: Row) => row.storage } }] : []),
    ...(visibleColumns.includes('size') ? [{ id: 'size', header: 'Original Size', accessorFn: (row: Row) => row.sizeBytes, size: 150, minSize: 130, maxSize: 240, cell: (info: any) => <span>{info.row.original.size}</span>, meta: { kind: 'number', title: (row: Row) => row.sizeTitle } }] : []),
    ...(visibleColumns.includes('storageSize') ? [{ id: 'storageSize', header: 'Storage Size', accessorFn: (row: Row) => row.storageSizeBytes, size: 150, minSize: 130, maxSize: 240, cell: (info: any) => <span>{info.row.original.storageSize}</span>, meta: { kind: 'number', title: (row: Row) => row.storageSizeTitle } }] : []),
    ...(visibleColumns.includes('media') ? [{ id: 'media', header: 'Type', accessorFn: (row: Row) => row.media, size: 145, minSize: 120, maxSize: 220, cell: (info: any) => <span>{info.row.original.media}</span>, meta: { kind: 'secondary', title: (row: Row) => row.media } }] : []),
    ...(visibleColumns.includes('backupMode') ? [{ id: 'backupMode', header: 'Backup Mode', accessorFn: (row: Row) => row.backupMode, size: 140, minSize: 120, maxSize: 190, cell: (info: any) => <span>{info.row.original.backupMode}</span>, meta: { kind: 'secondary', title: (row: Row) => row.backupMode } }] : []),
