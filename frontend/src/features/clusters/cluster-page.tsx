        ...
        <div className="hbdr-section-card hbdr-clusters-card-region">
          ...
          <div className={`hbdr-cluster-card-grid ${clusters.length <= 3 ? 'is-single-row' : 'is-multi-row'}`}>
            {clusters.map(cluster => (
              ...
              <p className="cluster-card-meta mb-2 text-[11px] font-medium text-slate-500"><span className={`font-semibold ${clusterTypePresentation(cluster.clusterType).textClassName}`}>{clusterTypePresentation(cluster.clusterType).label}</span> · Kubernetes {cluster.version} · {cluster.connectionStatus === 'online' ? 'Online' : 'Offline'}</p>
