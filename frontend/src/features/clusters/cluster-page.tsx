            ...
            >
              ...
              <div className="hbdr-filter-drawer-body hbdr-unregister-drawer-body">
                ...
                <section className="rounded-xl border border-slate-200 bg-white p-4 text-sm">
                  ...
                  {(unregisterPrecheck?.restorePointCount || unregisterPrecheck?.sourcePlanCount || unregisterPrecheck?.targetPlanCount) ? <label className="mt-3 flex items-start gap-2 border-t border-slate-100 pt-3 text-xs text-slate-600"><input type="checkbox" checked={deleteBackupData} onChange={event=>setDeleteBackupData(event.target.checked)} disabled={unregistering||forceRemoveEnabled} className="mt-0.5"/><span><strong className="block text-slate-800">Clean up related data</strong>{unregisterPrecheck.sourcePlanCount || unregisterPrecheck.targetPlanCount ? 'This cluster is used in DR relationships. Related DR relationships, restore points, backup data, and HyperCDR resources will be cleaned automatically.' : 'Related restore points and backup data will be cleaned automatically.'} Business namespaces and workloads will not be deleted.</span></label> : unregisterPrecheck?.objectStorageNeeded ? <p className="mt-3 border-t border-slate-100 pt-3 text-xs leading-5 text-blue-700"><strong className="block text-blue-800">Historical storage data detected</strong>The cluster storage prefix will be checked and cleaned automatically before unregister.</p> : <p className="mt-3 border-t border-slate-100 pt-3 text-xs text-emerald-700">This cluster has never used object storage. Object storage will not be accessed.</p>}
                </section>
              ...
              <div className="hbdr-filter-drawer-actions hbdr-unregister-drawer-actions">
                  ...
                  <button
                    disabled={unregistering || unregisterPrecheckLoading || (!forceRemoveEnabled && ((!unregisterPrecheck?.allowed && !deleteBackupData) || Boolean(unregisterPrecheck.restorePointCount && !deleteBackupData))) || (forceRemoveEnabled && (Boolean(unregisterPrecheck?.targetPlanCount) || forceRemoveConfirmation !== (unregisterTarget.name === 'unknown-cluster' ? unregisterTarget.id : unregisterTarget.name)))}
