...
export default function ActivityLogPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await apiGet<ApiList<ApiAuditLog>>('/api/v1/audit-logs?limit=500');
  ...
  const operationColumns: HyperTableColumn<typeof jobs[number]>[] = [
    { id: 'type', header: 'Operation', accessorFn: job => job.type, size: 190, minSize: 150, cell: info => <span>{info.row.original.type}</span>, meta: { kind: 'primary', title: job => job.type } },
    { id: 'object', header: 'Resource', accessorFn: job => job.object, size: 300, minSize: 180, maxSize: 520, cell: info => info.row.original.object, meta: { kind: 'secondary', title: job => job.object } },
    { id: 'status', header: 'Result', accessorFn: job => job.status, size: 130, minSize: 110, cell: info => <span className={info.row.original.status === 'Success' ? 'text-emerald-600' : 'text-rose-600'}>{info.row.original.status}</span>, meta: { kind: 'status', title: job => job.status } },
    { id: 'user', header: 'User', accessorFn: job => job.user, size: 150, minSize: 120, cell: info => info.row.original.user, meta: { kind: 'secondary', title: job => job.user } },
    { id: 'time', header: 'Time', accessorFn: job => job.time, size: 190, minSize: 150, maxSize: 280, cell: info => info.row.original.time, meta: { kind: 'secondary', title: job => job.time } },
  ];
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hbdr-history-page-title">
              <h3 className="text-sm font-black tracking-tight text-slate-900">Activity Log</h3>
              <p className="mt-0.5 text-[11px] font-medium text-slate-400">Review administrator actions, affected resources, and results.</p>
            </div>
          </div>
          <div />
        </div>
        ...
        <div className="hbdr-dr-table-head">
          <div className="hbdr-dr-toolbar">
            <div className="hbdr-dr-action-group">
              <button type="button" onClick={exportAudit} className="hbdr-dr-action-secondary">Export CSV</button>
            </div>
