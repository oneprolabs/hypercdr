...
export default function DiagnosticLogsPage({ currentUser, toast, advancedTenancy = false, initialTaskId = '' }: {
  ...
  return <motion.div key="diagnostic-logs" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
    ...
    <section className="hbdr-dr-table-card overflow-hidden">
      ...
      <HyperTable variant="page" density="comfortable" columns={columns} data={logs} getRowId={row => row.id} onRowClick={row => setSelected(row)} emptyMessage={loading ? 'Loading diagnostic logs...' : source === 'platform' ? 'No platform logs match the selected filters.' : 'No collected cluster logs match the selected filters.'} />
    </section>
