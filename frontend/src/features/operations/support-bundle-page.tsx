...
export default function SupportBundlePage({ toast }: { toast: (message: string) => void }) {
  ...
  const remove = async (bundle:Bundle) => {
    if (!window.confirm(`Delete support bundle ${bundle.name}?`)) return;
