...
export default function SupportBundlePage({ toast }: { toast: (message: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <AnimatePresence>{deleteTarget&&<><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="hbdr-filter-drawer-backdrop" onClick={()=>!deleting&&setDeleteTarget(null)}/><motion.aside initial={{opacity:0,x:34}} animate={{opacity:1,x:0}} exit={{opacity:0,x:34}} transition={{duration:.18,ease:'easeOut'}} className="hbdr-filter-drawer" role="dialog" aria-modal="true" aria-label="Delete support bundle"><div className="hbdr-filter-drawer-head"><div><strong>Delete Support Bundle</strong><span>This action cannot be undone.</span></div><button type="button" onClick={()=>setDeleteTarget(null)} disabled={deleting} aria-label="Close delete support bundle"><X size={18}/></button></div><div className="hbdr-filter-drawer-body"><section className="hbdr-advanced-filter-section"><h4>Confirmation</h4><div className="hbdr-advanced-filter-box"><p className="text-sm leading-6 text-slate-600">Delete <strong className="break-all text-slate-900">{deleteTarget.name}</strong>? The saved diagnostic archive will be removed from the platform.</p></div></section></div><div className="hbdr-filter-drawer-actions"><button type="button" disabled={deleting} onClick={()=>void remove(deleteTarget)}>{deleting?'Deleting…':'Delete Bundle'}</button><button type="button" disabled={deleting} onClick={()=>setDeleteTarget(null)}>Cancel</button></div></motion.aside></>}</AnimatePresence>
  </div>;
}
