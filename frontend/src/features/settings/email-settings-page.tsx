...
export default function EmailSettingsPage({ currentUser, toast }: { currentUser: ApiLoginResponse['user']; toast: (message: string) => void }) {
  ...
  const columns = useMemo<HyperTableColumn<ApiEmailSettings>[]>(() => [
    ...
    { accessorKey: 'isDefault', header: 'Default', size: 105, cell: ({ row }) => row.original.isDefault ? <span className="inline-flex items-center gap-1 font-medium text-blue-700"><Star size={12} fill="currentColor" />Yes</span> : 'No', meta: { kind: 'status' } },
    { accessorKey: 'name', header: 'Configuration Name', size: 190, cell: ({ row }) => <span className="truncate">{row.original.name}</span>, meta: { kind: 'primary', title: row => row.name } },
    ...(visibleColumns.includes('host') ? [{ accessorKey: 'host', header: 'SMTP Server', size: 190, cell: ({ row }: { row: { original: ApiEmailSettings } }) => <span>{row.original.host}</span>, meta: { kind: 'secondary', title: (row: ApiEmailSettings) => row.host } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('port') ? [{ accessorKey: 'port', header: 'Port', size: 90, meta: { kind: 'number' } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('security') ? [{ accessorKey: 'security', header: 'Encryption', size: 120, cell: ({ row }: { row: { original: ApiEmailSettings } }) => row.original.security.toUpperCase(), meta: { kind: 'secondary' } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('username') ? [{ accessorKey: 'username', header: 'Username', size: 170, cell: ({ row }: { row: { original: ApiEmailSettings } }) => row.original.username || '-', meta: { kind: 'secondary' } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('senderName') ? [{ accessorKey: 'senderName', header: 'Sender Name', size: 150, cell: ({ row }: { row: { original: ApiEmailSettings } }) => row.original.senderName || '-', meta: { kind: 'secondary' } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('senderEmail') ? [{ accessorKey: 'senderEmail', header: 'Sender Email', size: 210, meta: { kind: 'secondary', title: (row: ApiEmailSettings) => row.senderEmail } } as HyperTableColumn<ApiEmailSettings>] : []),
    ...(visibleColumns.includes('lastTestStatus') ? [{ accessorKey: 'lastTestStatus', header: 'Test Status', size: 130, cell: ({ row }: { row: { original: ApiEmailSettings } }) => <TestStatusBadge item={row.original} />, meta: { kind: 'status' } } as HyperTableColumn<ApiEmailSettings>] : []),
