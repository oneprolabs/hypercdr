...
func TestRegistrationRejectsTokenClusterTypeMismatch(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "cluster type") { t.Fatalf("expected cluster type mismatch, got %v", err) }
}
...
func TestLegacyRegistrationDefaultsToNativeKubernetes(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if cluster.ClusterType != "native-kubernetes" { t.Fatalf("cluster type = %q", cluster.ClusterType) }
}
