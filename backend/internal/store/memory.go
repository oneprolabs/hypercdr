...
func (s *MemoryStore) ListTasksFiltered(filter TaskFilter) ([]Task, error) {
	...
	for _, item := range s.tasks {
		if (filter.TenantID == "" || item.TenantID == filter.TenantID) &&
			...
			if filter.Summary {
				summary := map[string]any{}
				for _, key := range []string{"namespace", "sourceNamespace", "targetNamespace", "targetNamespaces", "targetClusterName", "applicationName", "stage", "recoveryStages", "archivedClusterId", "archivedClusterName", "restorePointId", "archivedRestorePointId", "pointId", "veleroBackupName", "backupName", "storageRepoId", "repositoryId", "storageRepo", "backupStorageName", "storageLocation", "repository", "name"} {
