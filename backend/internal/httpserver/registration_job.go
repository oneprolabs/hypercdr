...
func (r *Router) createRegistrationExecutorJob(ctx context.Context, taskID string) error {
	return r.createIsolatedRegistrationJob(ctx, "register-"+taskID, taskID, []any{map[string]string{"name": "HCDR_REGISTRATION_TASK_ID", "value": taskID}})
}

func (r *Router) createRegistrationInspectionJob(ctx context.Context, sessionID string, contextName string) error {
	return r.createIsolatedRegistrationJob(ctx, "inspect-"+sessionID, sessionID, []any{
...
func (r *Router) createIsolatedRegistrationJob(ctx context.Context, jobIdentity string, labelIdentity string, actionEnv []any) error {
	if r.cfg.DeployMode != "helm" && r.cfg.DeployMode != "kubernetes" {
		return nil
	}
	if r.cfg.RegistrationExecutorImage == "" || r.cfg.RegistrationConfigSecret == "" {
		return errors.New("Helm registration executor image or configuration Secret is not configured")
	}
	...
	job := map[string]any{"apiVersion": "batch/v1", "kind": "Job", "metadata": map[string]any{"name": "hypercdr-" + suffix, "namespace": r.cfg.RegistrationExecutorNamespace, "labels": map[string]string{"app.kubernetes.io/component": "cluster-registration-executor", "hypercdr.io/operation-id": labelIdentity}}, "spec": map[string]any{
		...
		"template": map[string]any{"metadata": map[string]any{"labels": map[string]string{"app.kubernetes.io/component": "cluster-registration-executor"}}, "spec": map[string]any{
			...
			"containers": []any{map[string]any{"name": "executor", "image": r.cfg.RegistrationExecutorImage, "imagePullPolicy": "IfNotPresent", "envFrom": []any{map[string]any{"secretRef": map[string]string{"name": r.cfg.RegistrationConfigSecret}}}, "env": append(actionEnv, map[string]string{"name": "HCDR_REGISTRATION_SESSION_DIR", "value": "/var/lib/hypercdr/registration-sessions"}), "volumeMounts": []any{map[string]any{"name": "sessions", "mountPath": "/var/lib/hypercdr/registration-sessions"}, map[string]any{"name": "tmp", "mountPath": "/tmp"}}, "securityContext": map[string]any{"allowPrivilegeEscalation": false, "readOnlyRootFilesystem": true, "capabilities": map[string]any{"drop": []string{"ALL"}}}, "resources": map[string]any{"requests": map[string]string{"cpu": "100m", "memory": "128Mi"}, "limits": map[string]string{"cpu": "1", "memory": "512Mi"}}}},
