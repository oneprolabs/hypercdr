...
Runtime files are stored outside the source tree under `/data/hypercdr-runtime/dev`.
...
`frontend/node_modules` symlink points to external dependencies under
`/data/hypercdr-runtime/dev/frontend` and is removed by `stop-dev.sh`.
...
The first start creates `/data/hypercdr-runtime/dev/dev.conf` from
