...
Usage:
  push-release.sh <version> --registry REGISTRY [--component all|api|frontend|agents|executor]
USAGE
}
...
case "${COMPONENT}" in
  ...
  api) images=(platform-api) ;;
  frontend) images=(platform-frontend) ;;
  agents) images=(comm-agent oadp-comm-agent) ;;
  executor) images=(cluster-registration-executor) ;;
  *) die "component must be all, api, frontend, agents, or executor" ;;
esac
REGISTRY="${REGISTRY%/}"
