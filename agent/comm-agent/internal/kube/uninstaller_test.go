...
func TestKubernetesUninstallerUsesNamespaceScopedRBACNames(t *testing.T) {
	const namespace = "hypercdr-enterprise-agent"
	client := fake.NewSimpleClientset(
		&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: namespace, UID: types.UID("enterprise-namespace-uid")}},
		&rbacv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-agent-9ced77ed"}},
		&rbacv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-agent-9ced77ed"}},
		&rbacv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-velero-9ced77ed"}},
		&rbacv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-velero-9ced77ed"}},
		&rbacv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-agent"}},
		&rbacv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-agent"}},
		&rbacv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-velero"}},
		&rbacv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "hypercdr-velero"}},
	...
	if err := uninstaller.Uninstall(context.Background(), UninstallOptions{Namespace: namespace, DeleteVelero: true, DeleteNamespace: true}); err != nil {
		t.Fatal(err)
	}
	for _, name := range []string{"hypercdr-agent", "hypercdr-velero"} {
		if _, err := client.RbacV1().ClusterRoles().Get(context.Background(), name, metav1.GetOptions{}); err != nil {
			t.Fatalf("community cluster role %s must be preserved: %v", name, err)
		}
		if _, err := client.RbacV1().ClusterRoleBindings().Get(context.Background(), name, metav1.GetOptions{}); err != nil {
			t.Fatalf("community cluster role binding %s must be preserved: %v", name, err)
		}
	}
}
...
func TestKubernetesUninstallerKeepsVeleroCRDsWhenExternalControllerExistsWithoutCRs(t *testing.T) {
	client := fake.NewSimpleClientset(&appsv1.Deployment{ObjectMeta: metav1.ObjectMeta{Name: "velero", Namespace: "hypercdr-enterprise-agent"}})
	crdGVR := schema.GroupVersionResource{Group: "apiextensions.k8s.io", Version: "v1", Resource: "customresourcedefinitions"}
	listKinds := map[schema.GroupVersionResource]string{}
	for _, gvr := range veleroNamespacedResources() {
		listKinds[gvr] = "UnstructuredList"
	...
	dynamicClient := dynamicfake.NewSimpleDynamicClientWithCustomListKinds(runtime.NewScheme(), listKinds,
		&unstructured.Unstructured{Object: map[string]any{
			...
			"metadata": map[string]any{"name": "backups.velero.io"},
		}},
	)
	uninstaller := NewKubernetesUninstallerWithClients(client, dynamicClient)
	if err := uninstaller.Uninstall(context.Background(), UninstallOptions{Namespace: "hypercdr-agent", DeleteVelero: true}); err != nil {
		t.Fatal(err)
	}
	if _, err := dynamicClient.Resource(crdGVR).Get(context.Background(), "backups.velero.io", metav1.GetOptions{}); err != nil {
		t.Fatalf("shared Velero CRD was deleted while another controller exists: %v", err)
	}
}
